...
from app.schemas import (
    ...
    ParseResponse, NotificationResponse, MessageResponse
)
...
def create_task(request: TaskCreateRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        task = service.create_task_from_natural_language(request.text)
...
def list_tasks(
    ...
    try:
        ...
        if completed is not None:
            query = db.query(service.db.query(Task)).filter(Task.completed == completed)
        else:
            query = service.get_all_tasks()
        
        # Simple filtering (in production, use more sophisticated query building)
        tasks = service.get_all_tasks()
        
        if completed is not None:
            tasks = [t for t in tasks if t.completed == completed]
        if priority:
            tasks = [t for t in tasks if str(t.priority.value if hasattr(t.priority, 'value') else t.priority) == priority]
        if category:
...
def get_upcoming_tasks(days: int = Query(7, ge=1, le=90), db: Session = Depends(get_db)):
    """
...
def update_task(task_id: str, request: TaskUpdateRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        # Update priority enum if provided
